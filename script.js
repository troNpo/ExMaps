...
.on('markgeocode', function(e) {
  ...
  marcadorBusquedaNominatim?.remove(); // Elimina el anterior si existe

marcadorBusquedaNominatim = L.marker(center)
  .addTo(map)
...
function actualizarBotones() {
  ...
  const panelPoiActivo = document.getElementById("panelPoi")?.classList.contains("visible");

  const hayPanelAbierto = sidebarActivo || subpanelActivo || panelPoiActivo;

  menuBtn.style.display = hayPanelAbierto ? "none" : "block";
  menuBtn.style.pointerEvents = hayPanelAbierto ? "none" : "auto";
...
function abrirEditorOSM() {
  ...
  // Abre el editor web de OSM en modo edición con zoom
...
function añadirNotaOSM() {
  ...
  // Texto sugerido que aparece prellenado en OSM
  const textoNota = encodeURIComponent(`Sugerencia para este lugar: ${nombre}`);

  // Construcción de URL con mapa centrado y mensaje
